# ldap-WebSecurityConfiguration-example.md
---
...
providerUserDn
域配置：
是否开启域设置：switchDomain  说明：开启域，自动显示下面三个字段，否则不显示走默认库验证登录
域链接： providerUrl  （ldap://host:port）
区分模式：userDnPattern
搜做组织：groupSearchBase
域密码： providerPassword
---
